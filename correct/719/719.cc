...
// Problem number: 719
// Description: Look for the first occurence of a prefix that is at least as
// long as the necklace in the sorted suffix array made with the string repeated
//*****************************************************************************
/*
 * Suffix array
 *
...
int main(void)
{
  ...
  for(int i = 0; i < cases; ++i) {
     getline(cin, str);
     circle = str + str;
     build_sarray(circle, sarray);
     string answer;
     string::size_type index;
     if(circle.size() > 0) {
	string::size_type j;
	for(j = 0; j < circle.length(); ++j) {
	   if(sarray[j] < str.length()) {
	      if(answer.size() != 0) {
 		 if(circle.substr(sarray[j], str.length()) == answer) {
		    if(sarray[j] < index) {
		       index = sarray[j];
		    }
		 } else {
		 ...
		 index = sarray[j];
		 answer = circle.substr(sarray[j], str.length());
	      }
	   }
