...
// Problem number: 11512
// Description: checks the longest common prefixes in made by the suffix array
// to see how many times the same suffix is repeated and then returns it along
...
int main(void)
{
  ...
  for(int i = 0; i < cases; ++i) {
	...
	if(lcp[j] > max) {
	   max = lcp[j];
	   occurances = 2;
	   repeat = str.substr(sarray[j - 1], lcp[j]);
	} else if(lcp[j] == max && max != 0 ) {
	   // check to see if the string is the same as before and increase
	   // the number of occurances
	   if(str.substr(sarray[j - 1], lcp[j]) == repeat) {
	      occurances++;
	   }
	...
	cout << repeat << " " << occurances << endl;
     }
  }
  return 0;
}

// compute the suffix array and the LCP array, and look for the maximum value
// in the LCP array

// if there is a tie, choose the first one (lexiographical order)
