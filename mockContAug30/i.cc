...
int main() {
   Point lov1, lov2, start, end, p;
   int res;
   double total_dist = 0;
   int cases = 1;
   double st = 0, e = 0;
   while(cin >> lov1 >> lov2 >> start >> end) {
      res = intersect_line(lov1, lov2, start, end, p); 
      if(res == 1) {
	 // find shorter side
	 st = dist(lov1, start) + dist(lov2, start);
	 e = dist(lov1, end) + dist(lov2, end);
	 if(st < e) {
	    total_dist = st;
	 } else {
	    total_dist = e;
	 ...
	 total_dist = dist(lov1, lov2);
      } else {
	 // don't know if this is possible (infinite number of intersections)
	 st = dist(lov1, start) + dist(lov2, start);
	 e = dist(lov1, end) + dist(lov2, end);
	 if(st < e) {
	    total_dist = st;
      ...
      cout << "Case " << cases++ << ": " << fixed << setprecision(3)
	   << total_dist/2.0 << endl;
   }
