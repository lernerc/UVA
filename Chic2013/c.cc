...
int getmin(int M[4*MAX_N], int a, int b, int st=0,int en=size,int ind=0) {
  if (a > b || a >= en || b < st)         return INT_MAX;
...
void doQuery(int i, int j, int place[], int lcp[], int M[]) {
   int z = min(place[i], place[j])+1;
   int q = max(place[i], place[j]);
   cout << getmin(M, z, q) << endl;
}
...
int main() {
   ...
   while(cin >> a && a != "*") {
	 ...
	 place[sarray[i]]=i;
      ...
      constructRMQ(lcp,M,a.size());
      int q, i, j;
      cin >> q;
      for(int z = 0; z < q; z++) {
	 cin >> i >> j;

	 doQuery(i,j, place, lcp, M);
      }
   }
   return 0;
}
